...
function attachPlugin(session, partyId, placeId, audioMute, videoMute, localStream) {
  return eventChannel(emitter => {
    ...
    session.attach({
        ...
        switch(event) {
          case 'joined': 
            ...
            if(msg['publishers']){
              ...
              Janus.debug(list);
              list.filter(feed => feed.display !== `actor:${placeId}`).map(feed => newRemoteFeed(emitter, session, mypvtid, partyId, feed, feeds));
            }
          break;
          case 'destroyed':
      ...
      onlocalstream: stream => {
        emitter({ type: ATTACH_LOCAL_FEED, feed: {
          sfutest,
          stream
        }});
      },
...
function publishOwnFeed(emitter, pluginHandle, useAudio, useVideo, localStream) {
    ...
    error: function(error) {
      ...
      if (useAudio) {
        publishOwnFeed(emitter, pluginHandle, false, useVideo, localStream);
      } else {
        reportError({
...
function newRemoteFeed(emitter, session, mypvtid, myroom, feedInfo, feeds) {
  ...
  session.attach(
      ...
      slowLink: (uplink, lost) => {
        ...
        Janus.debug('Remote feed', remoteFeed);
        Janus.debug(stream);
        emitter({
          type: RECEIVE_REMOTE_FEED,
          id: remoteFeed.rfdisplay,
          stream
        });
			},
			oncleanup: function() {
...
function setNewStream(sfutest, stream) {
...
function* watchForActions(sfutest) {
  try {
    while (true) {
      ...
      switch(action.type) {
        case SET_LOCAL_MUTES:
          sfutest.send({
            message: {
              request: 'configure',
              audio: !action.audioMute,
              video: !action.videoMute
            }
          })
          break;
        // case SET_LOCAL_STREAM:
        //   yield call(setNewStream, sfutest, action.stream)
        //   break;
      }
    }
  } catch(e) {
...
function* videoRoomSaga(action) {
  let { server, partyId, placeId, audioMute, videoMute, localStream } = action;
  ...
  try {
    ...
    let actionWatcher;
    try {
      let pluginChannel = yield call(attachPlugin, session, partyId, placeId, audioMute, videoMute, localStream);
      while(true) {
        let event = yield take(pluginChannel);
        console.log('JANUS EVENT', event);
        if(event.type === 'PLUGIN_ATTACHED') {
          actionWatcher = yield fork(watchForActions, event.sfutest);
        } else {
          yield put(event);
        }
      }
