        ...
        switch(event) {
          case 'joined': 
            ...
            if(msg["publishers"]){
              ...
              Janus.debug(list);
              list.filter(feed => feed.display !== `attendee:${userId}`).map(feed => newRemoteFeed(emitter, session, mypvtid, partyId, feed, feeds));
            }
          break;
          case 'destroyed':
...
function newRemoteFeed(emitter, session, mypvtid, myroom, feedInfo, feeds) {
  ...
  session.attach(
        ...
        // TODO: Handle this with an action
          ...
          if(remoteFeed.rfdisplay.startsWith('attendee:') || remoteFeed.rfdisplay.startsWith('guide:')) {
